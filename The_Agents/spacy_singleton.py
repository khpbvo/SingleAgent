...
class SpacyModelSingleton:
    ...
    @classmethod
    async def initialize(cls, model_name: str = "en_core_web_lg", disable: List[str] = None) -> 'SpacyModelSingleton':
        """
        ...
        if not cls._initialized:
            cls._logger.info(f"Loading spaCy model {model_name}...")
            ...
            try:
                ...
                cls._model = await loop.run_in_executor(
                    None,
                    lambda: spacy.load(model_name, disable=disable)
                )
                cls._initialized = True
                cls._logger.info(f"SpaCy model {model_name} loaded successfully")
                print(f"{GREEN}SpaCy model {model_name} loaded successfully{RESET}")
            except Exception as e:
                cls._logger.error(f"Failed to load spaCy model: {str(e)}")
    ...
    async def process_text(self, text: str) -> "spacy.tokens.Doc":
        """
    ...
    async def map_entity_types(self, entities: Dict[str, List[Dict[str, Any]]]) -> Dict[str, List[Dict[str, Any]]]:
        """
